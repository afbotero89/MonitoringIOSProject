...
class MonthCollectionCell: UICollectionViewCell, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let date = dates[(indexPath as NSIndexPath).item]
        
        cell.date = ((indexPath as NSIndexPath).item < dates.count) ? date : nil
        cell.mark = (selectedDate == date)
        
        cell.disabled = ((indexPath as NSIndexPath).item < previousMonthVisibleDatesCount) ||
                        ((indexPath as NSIndexPath).item >= previousMonthVisibleDatesCount
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        if monthCellDelgate != nil {
            monthCellDelgate!.didSelect(dates[(indexPath as NSIndexPath).item])
        }
    }
